...
namespace ProLEGO.Models
{
    public class DBUtility
    {
        ...
        private static SqlConnection GetOAConnector()
        {
            var conn = new SqlConnection();
            try
            {
                conn.ConnectionString = @"Server=wux-prod03\prod01;uid=OA_FAI_Reader;pwd=123_FAI_#;Database=OA;Connection Timeout=30;";
                conn.Open();
                return conn;
            }
            catch (SqlException ex)
         ...
         */
        public static List<List<object>> ExeOASqlWithRes(string sql, Dictionary<string, string> parameters = null)
        {
            //var syscfgdict = CfgUtility.GetSysConfig(ctrl);

            var ret = new List<List<object>>();
            var conn = GetOAConnector();
            try
            {
                if (conn == null)
                    return ret;
